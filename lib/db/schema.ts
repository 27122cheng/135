/**
 * The schema, embedded so `/api/setup` can create the tables without anyone
 * opening a SQL console.
 *
 * `supabase/schema.sql` stays the canonical file — this constant is a verbatim
 * copy, and tests/db.test.ts reads the file off disk and asserts they are
 * byte-identical. Edit the .sql file, then paste it here; the test fails loudly
 * if the two ever drift.
 *
 * A serverless bundle can't reliably read a repo file at runtime, which is why
 * this is a constant rather than an fs.readFile.
 */
export const SCHEMA_SQL = `-- Stage 2: persisted signal history for the Vercel Cron job and /history page.
 ...
 *
 * Neon's HTTP endpoint executes one statement per request, so the file can't
 * be sent as a single blob. Comments are stripped first because a trailing
 * `-- ...` would otherwise swallow the semicolon that ends its statement.
 *
 * This is safe only because the schema contains no dollar-quoted bodies
 * (functions, DO blocks) and no semicolons inside string literals — a naive
 * split would break on either. Keep it that way, or replace this with a real
 * parser.
 */
export function schemaStatements(sql: string = SCHEMA_SQL): string[] {
  return sql
    .split("\n")
