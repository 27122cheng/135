...
export async function analyzePositioning(symbol: string, gaps: string[]): Promise<PositioningResult> {
  const reports = await fetchCotReport(symbol, gaps);
  if (!reports || reports.length === 0) {
    return { biasItems: [], reports };
  }
  const items: BiasItem[] = [];
  const latest = reports.at(-1)!;
  const direction = latest.netNonCommercial > 0 ? "long" : latest.netNonCommercial < 0 ? "short" : "neutral";
  items.push({
    dimension: "籌碼面",
    factor: `CFTC COT 非商業淨部位 ${latest.netNonCommercial.toLocaleString()} 口 (${latest.reportDate})`,
    direction,
  ...
  if (window.length >= 10) {
    ...
    gaps.push(`CFTC COT (${symbol}) 歷史週數不足 10 週，無法判斷是否處於極端值`);
  }

  if (reports.length >= 2) {
    const prev = reports.at(-2)!;
    const weeklyChange = latest.netNonCommercial - prev.netNonCommercial;
    items.push({
      dimension: "籌碼面",
    ...
    gaps.push(`CFTC COT (${symbol}) 不足兩週資料，無法計算週變化`);
  }
