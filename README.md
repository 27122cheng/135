...
`stance = "enter"` 現在還要再過一關：**信心度 ≥ 90**。沒過就降為觀望，
...
**要知道的後果**：資料缺口最多扣 15 分，而只有 A+ 的基準（80）在 80 以上。
所以幾個來源掛掉的那一輪，無論型態多漂亮都過不了 90。這是刻意的讀法 ——
提高信心度最大的槓桿是把資料修好，不是把門檻調低。

測試同時釘住兩件事：乾淨的 A+ 加上有樣本的回測**確實過得了 90**
（一個永遠達不到的門檻等於把功能關掉），而同一組設定加上五項資料缺口就過不了。
...
`fetch`，不是真的打上去。22 個套件、578 項斷言，每個套件跑在自己的行程裡
