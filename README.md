...
也沒有理由尊重它。所以每一段加倉都落在**分析已經找到的結構**上：

- 交易方向上、進場與停利之間的 `path_obstacles` —— 走勢必須突破它們，
  每一次突破都是走勢自己證明了一次
- 突破後角色反轉：被突破的壓力在回測時就是支撐
...
`fetch`，不是真的打上去。18 個套件、378 項斷言，每個套件跑在自己的行程裡
...
| `monitor` | 加倉點必須錨定結構且停損只能收緊；監控狀態機不重複通知、模糊 K 棒一律報停損 |
