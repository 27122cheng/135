...
先把算術寫出來。損益兩平需要的風報比是 `(1 − 勝率) / 勝率`：

| 勝率 | 兩平所需風報比 | 意思 |
|---|---|---|
| 30% | 1:2.33 | 賺的要是賠的 2.33 倍 |
| 50% | 1:1 | |
| **80%** | **1:0.25** | **賭四塊賺一塊，一次虧損吃掉四次獲利** |
| 95% | 1:0.05 | 賭二十塊賺一塊 |

**高勝率永遠買得到，而且從來不免費。** 把停利拉到離進場夠近，95% 都做得出來——代價是
停損必須拉得極遠，於是整個優勢押在「不會連續虧兩次」上，而這不是任何一個市場具備的
性質。80% 配 1:0.25 時，連兩次停損等於吃掉八次獲利。

所以勝率不是一個可以「維持」的目標，它是停利距離的**結果**。系統做的是：先用 ATR 篩掉
不合理的停損與停利距離，再要求回測勝率 ≥ 30%，最後在通過的組合裡取期望值最高。實務上
會落在 35–55%、風報比 1:1.5 到 1:3——那是這些商品在 20 根 K 棒視窗下真正能拿到的東西。

如果之後真的想要更高的勝率，該調的是停利距離而不是門檻，而且卡片上會直接寫出那個勝率
對應的兩平風報比，不會讓「80% 勝率」看起來像免費的。

## 「近」不是百分比，是 ATR（`lib/analysis/proximity.ts`）

「這個結構離進場夠近嗎」原本寫死 1.5%，而且抄在四個地方。**價格的百分比不是市場感受得到
的距離，ATR 才是**。同一行程式在九個商品上差了四倍：

```
EURUSD  1.0800   1.5% = 162 pips  ≈ 2.7 × 日 ATR   太鬆
US30   53000     1.5% = 795 點    ≈ 1.6 × 日 ATR
WTI       70     1.5% = $1.05     ≈ 0.7 × 日 ATR   太緊
```

太鬆的那一端，讓價格要走好幾天才到得了的結構也算「保護進場」——結構分被灌水，停損還可能
錨在一個不相干的地方。太緊的那一端，把完全合理的結構丟掉——**這是某些商品一直沒有訊號的
原因之一**。

現在統一成 **2×ATR**：夠遠，讓一天區間之下的真實支撐仍然算數；夠近，讓一週外的價位不算。
ATR 是逐商品的，所以同一個常數在每個商品上意思相同。ATR 算不出來時才退回百分比，而且
卡片會說明用的是哪一條規則。

## 參考價位也走同一套選擇

觀望時卡片會顯示進場區、停損、一整排停利。那是**原始計算出來的價位**——進場區中點、最近
的保護結構、前方最近的障礙。**沒有任何東西在它們之間做過選擇**，所以卡片上唯一沒有經過
ATR 篩選、沒有回測、沒有期望值、沒有勝率的那一塊，正是被當成分析在讀的那一塊。

現在多一個「若要做，會是這一組」：跑完全相同的選擇流程，顯示進場／停損／停利、風報比、
**本地回測勝率與期望值**，以及被排除了哪些組合。唯一的差別是沒有人在建議它。

## 賠率不能表達的兩件事

線上跑出 US30 做多：進場 53,289.30、停損 53,160.77、停利 54,744.33、**風報比 1:11.32、
回測勝率 15%**。

這兩個數字是同一件事。停損距離 128 點，在一個日內區間好幾百點的指數上，**市場什麼都
不做就會穿過去**；停利 1,455 點外，不是 20 根 K 棒走得到的距離。

而期望值選擇抓不到，也不可能抓到：`0.15 × 11.32 − 0.85 = +0.85R`，在算術上贏過一個
正常的 1:2。**一個七次有六次被停損的正期望值，算術上沒問題，實務上不能用。**

問題在於「風報比」是無量綱的：1:11 完全沒有說停損是不是在一天的雜訊裡、停利是不是在
一個月外，而這兩個才是決定這筆能不能做的問題。所以加了三道**用 ATR 表達**的篩選：

| 篩選 | 門檻 | 理由 |
|---|---|---|
| 停損距離 | ≥ 0.6×ATR | 比這更近的不是停損，是對雜訊擲硬幣 |
| 停利距離 | ≤ 4×ATR | 回測視窗是 20 根，超過就不在證據涵蓋的範圍內 |
| 回測勝率 | ≥ 30% | 坐不住的優勢不是你會拿到的優勢 |

勝率那條是判斷，而且我把它當判斷寫出來：連續八次停損就是一個人不再照做的地方，而
一個會產生那種序列的系統就是一個會被關掉的系統。**先篩勝率，再取期望值最高**；如果
沒有任何組合過得了勝率門檻，才退回全部組合，並且在摘要裡直接寫「⚠ 沒有任何組合的回測
勝率達到 30%…部位要放小」，不會默默出貨。

被排除的組合數量與原因也寫進摘要。ATR 取不到時兩道距離篩選不執行——沒有基準的篩選是
在猜，而摘要會說它沒跑。

## 訊號被撤回時要出聲

08:54 的排程掃到 A+ 就發了 Telegram；下一輪掃到 no-trade，**什麼都沒說**。網站正確地
顯示比較新的那一列，所以上面什麼交易都沒有。沒有人搞錯，也沒有人被告知——這就是
「telegram 有交易但網站沒有」。

一個被撤回的建議必須**大聲撤回**，那是當初值得公告的同一份責任。現在前一次是可交易、
這一次不是時會發出撤回訊息（不是一張價位空白的交易卡，那會讀成「計畫缺數字」），內容
是本次評等、觀望理由、等待條件。

一直觀望的商品不會發，本來就沒到通知門檻的交易被撤回也不會發。

## 一個寫入失敗不該讓另一個不寫

`storeScan` 第一版在 `saveLatest` 失敗時直接 return，於是**歷史時間軸整個被跳過**——
`latest_signal` 有問題就等於什麼都沒存，總覽凍在最後一次成功的那一列，而排程還在照跑
照發通知。重構前 refresh 是先寫歷史、再吞掉 upsert 的錯誤，所以這個重構把一個**降級**
狀態變成了**靜默**狀態。

兩張表現在各寫各的、各自回報錯誤：時間軸是紀錄，目前訊號是視圖，其中一個壞掉不是跳過
另一個的理由。
...
網站是你去找它，推播是它來找你——兩者該有不同的門檻。現在**至少 3 個面向同向**才會推
Telegram：一個面向是一個指標，兩個可能是同一個總經故事講兩次，三種獨立證據同向才叫
共振，而共振才值得打斷你。

未達門檻的訊號**照常存在、照常顯示在網站上**（總覽會看到交易，手機不會響）——這是設計，
不是漏發。撤回通知也套同一條：當初沒推播的交易，消失時也不會響。

## 報價來源冗餘

Yahoo 直連曾經連續多日回傳四天前的舊報價（K 棒代理同時間是新的）。一條資料源不叫冗餘：
現在直連報價**缺失或超過 3 小時**就改問 K 棒代理的最新 hourly bar，帶自己誠實的時間戳。
兩邊都掛才回傳標示過的舊值——標示過的舊價仍然勝過沒有價。

## 實際命中率（復盤頁）

`[自動追蹤]` 的日誌現在有一張表把三條流分開統計，混在一起會互相灌水：

| 來源 | 回答的問題 |
|---|---|
| 正式訊號（自動追蹤） | 該不該信這個系統 —— **就是這個數字** |
| 參考價位（紙上追蹤） | 進場門檻擋掉的是贏家還是輸家 |
| 人工記錄 | 你自己的紀律 |

紙上追蹤假設完美成交，勝率讀作上限。**如果紙上長期贏過正式訊號，代表門檻把贏家擋掉了
該回頭調；如果輸，代表門檻在賺它的位置。** 兩個答案都有用，所以兩個數字並排。
...
訊號引擎一次只看一筆設定；這四條看的是「這張單放進整本帳」之後的事。全部只會
收回或縮小建議，不會創造建議，也全部只讀真實自動追蹤的日誌列（紙上追蹤假設完美
成交，不算錢）。
...
實驗室要的是十年日線或兩年小時線，免費來源只有 Yahoo 與 Stooq 不用金鑰，而 Stooq
拒絕機房 IP、Yahoo 的 `XAUUSD=X` 又是最常整天沒資料的代號，所以黃金的實驗長期卡在
「取不到 K 棒」。`lib/data-sources/deep-history.ts` 現在對黃金多試 `GC=F`（COMEX 期貨
連續合約）：同一個市場差一個基差，實驗以 ATR 為單位量幾何、以期望值排序條件，水位
平移不影響結果。用到時會在注意事項寫明；報價與價位一律仍用現貨。

### 每一種出場都算數

停損、停利、分批止盈、保本出場、結構翻轉提早出場、論點失效提早出場，全部進同一張
`trade_journal`，全部計入勝率、期望值、權益曲線與最大回撤。`lib/journal/exit-kind.ts`
把它們並排成「出場方式」表（復盤頁與週報都有），每種各自的勝負與損益一目了然。

提早出場的**虧損**同樣走 S1–S8 分類（`lib/journal/auto-log.ts`）：分類器看的是最大
順向幅度與事件視窗，跟最後一根是停損還是市價單無關。所以結構翻轉在停損前先來的
虧損會進停損原因分布，也會計入干涉規則；獲利與打平的提早出場不分類，沒有東西要修。

週報自此固定包含：正式訊號勝率與損益兩平線、獲利與虧損分開的金額、出場方式、本週
最痛的停損原因加上對應建議與系統是否已自動收緊、出場方式的建議、累計損益與最大回撤。

紙上追蹤的停損不教方向：S1、S7、S8 只計真實列（`lib/journal/interventions.ts`
`DIRECTION_TAGS`），S2、S3、S4 紙上列照計 —— 參考價位是閘門擋下來的訊號，它的停損證明
閘門是對的，不該再拿去提高全系統的方向門檻。

總曝險上限在 `lib/analysis/sizing.ts`：同時真實持倉達 4 筆即拒絕新倉；帳上未保本
的風險合計（以 R 計，停損在進場價的部位算 0）達 4R 即拒絕，剩餘額度不足 1R 則
按比例縮小。帳戶規模只存在瀏覽器，所以這一層算在部位卡上，不在伺服器。
...
建議推播後六十秒內不判定成交：成交只看建議發出一分鐘之後的價格，補看漏掉區間的
起點也以此為界，建議發出前的回踩永遠不算成交。卡片本身只留該有的：方向與評等、
三個價位與各自的百分比、風報比、停損距離（部位 = 可承受虧損 ÷ 停損距離）、一分鐘
規則、掛單有效期限，然後各一行的理由、新聞、實測、進場後的管理規則。其餘在網站。

每一張單固定兩則：先是交易建議（進場、停損、停利、理由），之後價格回踩到進場價
才是「已觸及進場價」。建議那一則由每小時掃描發送；監控在開始追蹤一張單時會確認
它的建議真的發出去過，沒有的話（產生時休市、當時另有持倉、或掃描判定「與上次相
同」而上次其實沒發）由監控補發一次，再開始等成交。兩邊共用一個先寫先贏的記錄，
不會重複；每小時重寫但價位相同的計畫繼承同一個旗標，不會每小時再發一次。
